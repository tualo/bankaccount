...
class TualoFinTS {
    ...
    public static function handleRequest($request, FinTs $fints,$db,&$persistedAction)
    {
        ...
        switch ($request['action']) {
            ...
            case 'getStatements':
                ...
                $to = new DateTime();
                $oneAccount = $getAccounts->getAccounts()[0];
                $getStatement = GetStatementOfAccount::create($oneAccount, $from, $to);
                ...
                foreach ($soa->getStatements() as $statement) {
                    ...
                    foreach ($statement->getTransactions() as $transaction) {
                        $factor = ($transaction->getCreditDebit() == Statement::CD_DEBIT ? -1 : 1);
                        ...
                        $bookingdate = $transaction->getDate()->format('Y-m-d');
